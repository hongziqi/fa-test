...
# from test_fa_fwd_prof_gpu import test_results, RESULT_DIR  # 导入你的全局变量

from test_fa_fwd_gpu_prof_tune import test_results, RESULT_DIR  # 导入你的全局变量
...
def pytest_sessionfinish(session, exitstatus):
    ...
    #     result_file = os.path.join(RESULT_DIR, f"test_results_gpu_tune_{timestamp}.xlsx")
    #     print(">> Length of test_results",len(test_results))
    ...
    try:
        result_file = os.path.join(RESULT_DIR, "test_results_gpu_tune_batch.xlsx")
        df = pd.DataFrame(test_results).fillna("")
